...
def plot_stock_data(ticker, data):
    ...
    ax[0].plot(data.index, data['EMA'], label='EMA', color='r')
    ax[0].plot(data.index, data['MACD'], label='MACD', color='g')
    ax[0].plot(data.index, data['Signal'], label='Signal Line', color='b')
    ax[0].plot(data.index, data['VWAP'], label='VWAP', color='y')
    ax[0].legend()
...
def display_articles_with_analysis(articles_with_tickers):
    ...
    for article, tickers in articles_with_tickers:
        ...
        # Technical Analysis
        try:
            ticker_data = yf.download(tickers[0], period="5d", interval="5m")  # Adjusted period to 5d for intraday data
